...
<head>
    ...
    <title>@foreach ($lab_records as $key=>$lab_record)
            @if ($key>0)
                @break
            @endif
            {{ucfirst($lab_record->labratory->name)}}
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <div class="header-title">
                <h1>Laboratory</h1>
                <small>@foreach ($lab_records as $key=>$lab_record)
                        @if ($key>0)
                            @break
                        @endif
                        {{ucfirst($lab_record->labratory->name)}}
                ...
                <div class="col-sm-12">
                    <div class="panel panel-bd lobidrag">
                        <div class="panel-heading">
                            <div class="btn-group">
                                <a class="btn btn-success" href="#"> @foreach ($lab_records as $key=>$lab_record)
                                        @if ($key>0)
                                            @break
                                        @endif
                                        {{ucfirst($lab_record->labratory->name)}}
                            ...
                            <div class="table-responsive">
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    @foreach($lab_records as $lab_record)
                                        <tr>
                                            <td>{{ucfirst($lab_record->labratory->name)}}</td>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="panel panel-bd lobidrag">
                        <div class="panel-heading">
                            <div class="btn-group">
                                <a class="btn btn-success" href="#"> @foreach ($payments as $key=>$payment)
                                        @if ($key>0)
                                            @break
                                        @endif
                                        {{ucfirst($payment->labratory->name)}}
                            ...
                            <div class="table-responsive">
                                <table id="dataTable1" class="table table-bordered table-hover">
                                    ...
                                    @foreach($payments as $payment)
                                        <tr>
                                            <td>{{ucfirst($payment->labratory->name)}}</td>
                                            <td>{{$payment->loan_amount}}</td>
                                            <td>{{$payment->pay_amount}}</td>
                                            <td>{{$payment->new_loan_amount}}</td>
                                            <td>{{$payment->date}}</td>
                                            <td>
                                                <form action="{{route('laboratory_payment.destroy',$payment->id)}}" method="POST" >
                                                    <input type="hidden" name="_method" value="delete">
                                                    {!! csrf_field() !!}
                                                    <button onclick="return confirm('Are you sure?')" type="submit" class="close-icon btn btn-primary btn-outline" ><i class="fa fa-remove"> </i></button>
                                                </form>
